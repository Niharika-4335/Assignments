...
//        saveCoursesAndReviews(instructorDao);
//        getCoursesAndReviews(instructorDao);
        deleteCoursesAndReviews(instructorDao);

    }

    private void deleteCoursesAndReviews(InstructorDao instructorDao) {
        instructorDao.deleteCourseAndReviews(10);
    }

    private void getCoursesAndReviews(InstructorDao instructorDao) {
        Course course=instructorDao.showCourseAndReviews(10);
        System.out.println(course.getReviews());


    }

    private void saveCoursesAndReviews(InstructorDao instructorDao) {

        Course course=new Course("basics of programming");
        course.addReview(new Review("very good"));
        course.addReview(new Review("knowledgeable"));
        course.addReview(new Review("very bad"));
        instructorDao.saveCourseAndReviews(course);
